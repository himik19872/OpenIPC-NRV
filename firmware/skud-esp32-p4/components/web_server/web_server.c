...
static esp_err_t status_get_handler(httpd_req_t *req)
{
    ...
    cJSON_AddStringToObject(root, "device_name", s_sys_current.device_name);
    cJSON_AddStringToObject(root, "device_location", s_sys_current.device_location);
    /* Версия и дата сборки: по ним видно, применилось ли OTA-обновление. */
    cJSON_AddStringToObject(root, "fw_version", FW_VERSION_STRING);
    cJSON_AddStringToObject(root, "fw_build", FW_BUILD_DATE);
...
/* Буфер приёма OTA вынесен из стека в статическую память.
 *
 * Обработчик работает в задаче HTTP-сервера, и 4 КБ на стеке заметно
 * сокращают её свободное место на время записи во flash. Статический
 * буфер безопаснее: OTA обрабатывается строго одним запросом за раз. */
static uint8_t s_ota_buf[4096];
...
static esp_err_t ota_upload_handler(httpd_req_t *req)
{
    ...
    /* Проверяем размер до начала записи: образ больше OTA-раздела
     * контроллер всё равно не примет, а стирание раздела и приём
     * мегабайта впустую занимают время. */
    if (req->content_len <= 0) {
        cJSON *e = cJSON_CreateObject();
        cJSON_AddBoolToObject(e, "ok", false);
        cJSON_AddStringToObject(e, "error", "empty_body");
    ...
    ESP_LOGI(TAG, "OTA: принимаю %d байт", req->content_len);

    /* stream body in chunks */
    int remaining = req->content_len;
    int total = req->content_len;
    uint8_t *buf = s_ota_buf;
    bool failed = false;
    while (remaining > 0) {
        int to_read = remaining < (int)sizeof(buf) ? remaining : (int)sizeof(buf);
        int got = httpd_req_recv(req, (char *)buf, to_read);
        if (got <= 0) {
            /* Отрицательное значение — ошибка сокета или таймаут приёма.
             * Логируем: без этого обрыв соединения выглядел бы как
             * обычная неудачная запись, и причину не найти. */
            ESP_LOGE(TAG, "OTA: приём прерван на %d из %d байт (код %d)",
                     total - remaining, total, got);
            failed = true;
            break;
        }
        if (ota_write(buf, got) != ESP_OK) {
            ESP_LOGE(TAG, "OTA: ошибка записи после %d байт", total - remaining);
            failed = true;
            break;
        }
        remaining -= got;
    ...
    ESP_LOGI(TAG, "OTA: образ принят целиком (%d байт), применяю", total);

    /* Ответ отправляем до ota_commit: тот перезагружает устройство, и
     * ответить после него уже не получится. Так сервер видит подтверждение
     * приёма, а не обрыв соединения при перезагрузке. */
    cJSON *ok = cJSON_CreateObject();
    cJSON_AddBoolToObject(ok, "ok", true);
    cJSON_AddNumberToObject(ok, "bytes", total);
    esp_err_t sent = send_json(req, ok, 200);
...
/* GET /api/ota/journal -> результат прошлой попытки OTA.
 *
 * Отдаётся по HTTP, потому что при сбое обновления устройство
 * перезагружается, и консольный лог теряется. */
static esp_err_t ota_journal_handler(httpd_req_t *req)
{
    if (!check_auth(req)) return auth_fail(req);

    char stage[64] = {0};
    char detail[128] = {0};
    ota_peek_journal(stage, sizeof(stage), detail, sizeof(detail));
...
static httpd_handle_t start_server(void)
{
    ...
    /* OTA принимает образ около мегабайта и обрабатывается в задаче
     * сервера. Настройки по умолчанию рассчитаны на короткие JSON-запросы:
     * стек 4 КБ и таймаут приёма 5 секунд. Обработчик OTA держит на стеке
     * буфер приёма и структуры cJSON, поэтому стек увеличен с запасом:
     * при 8 КБ запрос OTA приводил к сбою устройства. */
    cfg.stack_size = 16384;
    cfg.recv_wait_timeout = 30;
    cfg.send_wait_timeout = 30;
    cfg.keep_alive_enable = false;
    /* Больше сокетов и приоритет задачи выше: во время OTA контроллер не
     * должен переставать отвечать на запросы статуса, иначе проверка
     * версии после обновления не увидит, что устройство поднялось. */
    ...
    if (httpd_start(&h, &cfg) == ESP_OK) {
        ...
        REG("/api/ota/journal", HTTP_GET, ota_journal_handler);
