...
| **Мобильное приложение** | Android-приложение: выбор сервера по IP/порту/DNS-имени, онлайн-просмотр камер и архив записей. Подробности — [docs/MOBILE.md](docs/MOBILE.md) |
...
Android-приложение для телефона: подключение к серверу по IP-адресу, порту
или DNS-имени, онлайн-просмотр камер и работа с архивом.
...
| **Архив** | Список записей с фильтром по камере, встроенный плеер с перемоткой, подгрузка при прокрутке |
...
Готовый APK — около 51 МБ (только `arm64-v8a`, для современных телефонов).
...
В приложении пока нет детекции, событий, СКУД, PTZ и настроек камер — это
инструмент просмотра. Управление остаётся в веб-интерфейсе.
...
| Мобильное приложение | React Native, ExoPlayer | Просмотр камер и архива с телефона Android |
...
webui/              React SPA
mobile/             Android-приложение на React Native
  src/net/          клиент API и разбор адреса сервера
  src/screens/      экраны: серверы, вход, камеры, просмотр, архив
